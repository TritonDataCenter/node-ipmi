...
function
decode_packet_old(buf)
{
	if (buf.length < 8) {
		throw (new Error('too short'));
	}

	var format = buf.readUInt8(0);
	var fstr;
	...
	case IPMI_AUTHTYPE_PASSWORD:
		fstr = 'password';
		break;

	case IPMI_AUTHTYPE_RMCPPLUS:
		throw (new Error('IPMI 2.0 not yet implemented'));

	default:
		throw (new Error('IPMI authtype unknown: ' + format));
	}

	var seq_num = buf.readUInt32LE(1);
	var session_id = buf.readUInt32LE(5);

	var pos = 9;
	var auth_code;
	if (format !== IPMI_AUTHTYPE_NONE) {
		auth_code = buf.slice(pos, 24);
		pos += 16;
	}

	var msg_len = buf.readUInt8(pos);
	pos += 1;

	var data = buf.slice(pos);

	/*
	 * XXX need potentially lose the PAD byte.
	 * WTF is a "1-based" length?
	 */

	if (msg_len !== data.length) {
		throw (new Error('IPMI msglen %d !== actual len %d',
		    msg_len, data.length));
	}
	...
	return ({
		ipmi_version: '1.5',
		ipmi_authtype: fstr,
		ipmi_sequence: seq_num,
		ipmi_session_id: session_id,
		ipmi_auth_code: auth_code,
		ipmi_len: msg_len,
		ipmi_data: data
	});
}
...
function
encode_packet_old(ipmi)
{
	var format;

	if (ipmi.ipmi_version !== '1.5') {
		throw (new Error('only IPMI v1.5 supported'));
	}

	switch (ipmi.ipmi_authtype) {
	case 'none':
		format = IPMI_AUTHTYPE_NONE;
		mod_assert.ok(!ipmi.ipmi_auth_code, 'ipmi_auth_code, but NONE');
		break;

	case 'md5':
		format = IPMI_AUTHTYPE_MD5;
		mod_assert.ok(Buffer.isBuffer(ipmi.ipmi_auth_code));
		break;

	case 'password':
		format = IPMI_AUTHTYPE_PASSWORD;
		mod_assert.ok(Buffer.isBuffer(ipmi.ipmi_auth_code));
		break;

	default:
		throw (new Error('invalid authtype: ' + ipmi.ipmi_authtype));
	}

	mod_assert.number(ipmi.ipmi_sequence, 'ipmi_sequence');
	mod_assert.number(ipmi.ipmi_session_id, 'ipmi_session_id');
	mod_assert.ok(Buffer.isBuffer(ipmi.ipmi_data, 'ipmi_data'));

	var buf = new Buffer(10 + (ipmi.ipmi_authtype === 'none' ? 0 : 16) +
	    ipmi.ipmi_data.length);
	buf.fill(0x33);

	var pos = 0;
	buf.writeUInt8(format, pos);
	pos += 1;

	buf.writeUInt32LE(ipmi.ipmi_sequence, pos);
	pos += 4;

	buf.writeUInt32LE(ipmi.ipmi_session_id, pos);
	pos += 4;

	if (ipmi.ipmi_authtype !== 'none') {
		mod_assert.ok(Buffer.isBuffer(ipmi.ipmi_auth_code));
		mod_assert.strictEqual(ipmi.ipmi_auth_code.length, 16);

		pos += ipmi.ipmi_auth_code.copy(buf, pos);
	}

	mod_assert.ok(ipmi.ipmi_data.length < 255);
	buf.writeUInt8(ipmi.ipmi_data.length, pos);
	pos += 1;

	ipmi.ipmi_data.copy(buf, pos);
	pos += ipmi.ipmi_data.length;
